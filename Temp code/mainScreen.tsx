...
export default function App(): JSX.Element {
  ...
  /* -------------------------------------------------------------- */
  /*  socket + firebase  (FIXED REAL-TIME UPDATES)                  */
  useEffect(() => {
    let isMounted = true;
    ...
    const connectSocket = () => {
      ...
      socket.on("initialData", (d) => {
        console.log("Initial socket data:", d);
        if (!isMounted) return;

        setBuses((prevBuses) => {
          const updatedBuses = prevBuses.map((bus) => {
            const socketData = d[bus.busId];
            if (socketData) {
              return {
                ...bus,
                ...socketData,
                passengers:
                  socketData.passengers !== undefined
                    ? socketData.passengers
                    : bus.passengers,
          ...
          console.log("Updated buses after socket initial:", updatedBuses);
          return updatedBuses;
        });
      });
            ...
            prev.map((bus) =>
              bus.busId === busId
                ? {
                    ...
                    lon,
                    passengers:
                      passengers !== undefined ? passengers : bus.passengers,
                    status,
                    lastUpdateSource: "API",
                    lastUpdated: Date.now()
                  }
    ...
    // Periodic API fetch (every 10 seconds as backup)
    const fetchInterval = setInterval(async () => {
      if (!isMounted) return;

      try {
        const r = await fetch("http://localhost:4002/api/update-bus");
        if (!r.ok) return;
        ...
        setBuses((prevBuses) => {
          const updatedBuses = prevBuses.map((bus) => {
            const apiData = d[bus.busId];
            if (apiData) {
              console.log(`Updating bus ${bus.busId} with API data:`, apiData);
              return {
                ...bus,
                ...apiData,
                passengers:
                  apiData.passengers !== undefined
                    ? apiData.passengers
                    : bus.passengers,
                lastUpdateSource: "API",
                lastUpdated: Date.now()
          ...
          console.log("Updated buses after API:", updatedBuses);
          return updatedBuses;
        });
      } catch (error) {
  ...
  useEffect(() => {
    try {
      const bRef = ref(firebaseDb, "buses");
      const rRef = ref(firebaseDb, "routes");
      const u1 = onValue(bRef, (s) => {
        const v = s.val();
        console.log("Firebase buses data:", v);

        if (v) {
          const firebaseBuses = Object.keys(v).map((k) => ({
            ...v[k],
            busId: k
          }));

          setBuses((prevBuses) => {
            // If we already have buses with real-time data, preserve the passenger count
            const prevBusesMap = new Map(prevBuses.map((b) => [b.busId, b]));

            const mergedBuses = firebaseBuses.map((firebaseBus) => {
              const prevBus = prevBusesMap.get(firebaseBus.busId);

              // If we have recent real-time data, preserve passenger count
              if (
                prevBus &&
                prevBus.lastUpdateSource === "API" &&
                prevBus.lastUpdated &&
                Date.now() - prevBus.lastUpdated < 30000
              ) {
                console.log(
                  `Preserving real-time data for bus ${firebaseBus.busId}`
                );
                return {
                  ...firebaseBus,
                  ...
                  status: prevBus.status ?? firebaseBus.status,
            ...
            console.log("Merged buses after Firebase:", mergedBuses);
            return mergedBuses;
          });
        } else {
          setBuses([]);
      ...
      const u2 = onValue(rRef, (s) => {
        const v = s.val();
        setRoutes(
          v ? Object.keys(v).map((k) => ({ ...v[k], routeId: k })) : []
        );
      });

      return () => {
        typeof u1 === "function" && u1();
        typeof u2 === "function" && u2();
      };
    } catch (error) {
  ...
  const visibleBuses = useMemo(() => {
    const on = activeBuses.filter((b) => b.status === "online");
    if (!selectedRoute) return on;
    return on.filter(
      (b) =>
        b.routeId?.toString().trim() ===
        selectedRoute.routeId?.toString().trim()
    );
  }, [activeBuses, selectedRoute]);

  /* -------------------------------------------------------------- */
  /*  NEW SEARCH HANDLER  –  uses highway geometry                  */
  const handleSearchResult = async (
    ...
    if (start && end) {
      ...
      if (geom && geom.length) segment = geom;
    }
    ...
    setSearchMessage("");
    if (mapRef.current && segment.length > 1) {
      try {
        mapRef.current.fitBounds(L.latLngBounds(segment), {
          padding: [80, 80]
        });
      } catch {}
    }
  ...
  const handleSelectBus = (bus: Bus) => setSelectedBusId(bus.busId || null);

  /* -------------------------------------------------------------- */
  /*  UI  (UNCHANGED STYLES / STRUCTURE)                            */
  return (
    <div className="container">
      ...
      >
          ...
          <div className="sidebarContent">
            ...
            {selectedRoute && (
              ...
              >
                ...
                <div style={{ fontSize: 15, color: "#e9d5ff" }}>
                  <InfoRow
                    label="Route Name"
                    value={
                      selectedRoute.name ||
                      ...
                      {visibleBuses.map((bus) => (
                        ...
                        >
                          ...
                          <span style={{ color: "#10b981" }}>
                            {getLatestPassengerCount(bus) !== undefined
                              ? `${getLatestPassengerCount(bus)} passengers`
                              : "Active"}
                          </span>
                        </div>
                      ))}
        ...
        <main className="mapContainer">
          ...
          >
            ...
            {/* selected route – highway geometry */}
            {selectedRoute && highlight.length > 0 && (
              <Polyline
                  ...
                  <Popup>
                    ...
                    >
                      ...
                      >
                        ...
                        {getLatestPassengerCount(b) !== undefined && (
                          <div
                            style={{
                              padding: 8,
                              background: "#f0fdf4",
      ...
      <style>{`
        ...
        button:hover:not(:disabled) { transform: translateY(-2px) scale(1.02); box-shadow: 0 16px 40px rgba(139, 92, 246, 0.5), inset 0 1px 0 rgba(255, 255, 255, 0.2) inset !important; }
  ...
  return (
    ...
    >
      <path
        ...
        strokeLinejoin="round"
        strokeWidth={2}
        d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0"
      />
    </svg>
  );
}
...
function BusCard({
  ...
  // Debug log
  console.log(
    `BusCard ${bus.busId}: passengerCount =`,
    passengerCount,
    "bus.passengers =",
    bus.passengers
  );
...
/* ------------------------------------------------------------------ */
/*  SEARCH PANEL  (UNCHANGED – ALREADY WORKS WITH NEW HANDLER)      */
function SearchPanel({
  ...
  routes: Route[];
  buses: Bus[];
  onSearchResult: (r: any) => void;
  onLocateMe?: (l: LatLon | null) => void;
}): JSX.Element {
  ...
  useEffect(() => {
    ...
    routes.forEach((r) => {
      if (r.place1) set.add(r.place1.trim());
      if (r.place2) set.add(r.place2.trim());
      if (r.name)
        r.name
          .split(/[-–—]|to/i)
          .map((p) => p.trim())
    ...
    setDbLocations(Array.from(set).sort());
  }, [routes]);
  ...
  const selectStartSuggestion = (s: string) => {
    setStartText(s);
    setShowStartDropdown(false);
  };
  const selectEndSuggestion = (s: string) => {
    setEndText(s);
  ...
  const searchRoute = async () => {
    ...
    try {
      ...
      const startName = usingMyLoc ? "My Location" : startText.trim();
      const endName = endText.trim();

      // find matching route (bidirectional)
      const matching = routes.filter((r) => {
        const p1 = (r.place1 || "").toLowerCase().trim();
        const p2 = (r.place2 || "").toLowerCase().trim();
        const s = startName.toLowerCase();
        const e = endName.toLowerCase();
        return (
          (p1.includes(s) && p2.includes(e)) ||
          (p1.includes(e) && p2.includes(s)) ||
      ...
      if (!matching.length) {
        onSearchResult({
          status: "not_found",
          message: `No matching route between "${startName}" and "${endName}".`
        });
        setLoading(false);
        return;
      }
      ...
      const segment: Array<[number, number]> =
        stops.length >= 2
          ? stops.map((p) => [p.lat, p.lon] as [number, number])
          : [
              [start.lat, start.lon],
              [end.lat, end.lon]
            ];
    ...
    <div className="panel">
        ...
        <input
          ...
          onFocus={() => {
            if (!usingMyLoc) {
              const filtered = (
                startText.length
                  ? dbLocations.filter((l) =>
                      l.toLowerCase().includes(startText.toLowerCase())
                    )
                  : dbLocations
              ).slice(0, 10);
              setStartSuggestions(filtered);
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        <button
          ...
          disabled={loading}
          className="button"
          style={{ padding: "10px 12px", width: "auto", fontSize: 13 }}
        >
          <svg
            style={{ width: 18, height: 18 }}
            fill="none"
        ...
        <input
          ...
          onFocus={() => {
            const filtered = (
              endText.length
                ? dbLocations.filter((l) =>
                    l.toLowerCase().includes(endText.toLowerCase())
                  )
                : dbLocations
            ).slice(0, 10);
            setEndSuggestions(filtered);
...
function getLatestPassengerCount(bus: Bus): number | undefined {
  ...
  // Debug log
  console.log(`getLatestPassengerCount for bus ${bus.busId}:`, passengers);

  if (passengers === undefined || passengers === null) {
    console.log(`Bus ${bus.busId}: passengers is null/undefined`);
    return undefined;
  }
  if (typeof passengers === "number") {
    console.log(`Bus ${bus.busId}: passengers is number =`, passengers);
    return passengers;
  }
  if (typeof passengers === "object") {
    const entries = Object.entries(passengers);
    console.log(
      `Bus ${bus.busId}: passengers is object with entries:`,
      entries
    );
    if (entries.length) {
      ...
      console.log(`Bus ${bus.busId}: latest passenger count =`, latest);
      return typeof latest === "number" ? latest : undefined;
    }
  }
  console.log(`Bus ${bus.busId}: returning undefined`);
  return undefined;
}
